package caseofgames.quantumsystem.util;
 ...
 *
 * Gives you greater control over the Forge logger in case you need it. Thanks
 * to Darkhax Stuff-a-sock-in-it:
 * https://github.com/Darkhax-Minecraft/Stuff-A-Sock-In-It/tree/1.14.4
 *
 * Usage: 1) call setMinimumLevel() to change the minimum level of error
 * messages to be printed 2) call applyLoggerFilter() 3) can call
 * setMinimumLevel() again to change while the mod is running
 *
 * 4) optional: add custom logic to MBELogFilter.filter() if you want to filter
 * based on other criteria (such as particular named class loggers)
 *
 * Thanks Darkhax for letting me 'borrow' your code. :D
 */
